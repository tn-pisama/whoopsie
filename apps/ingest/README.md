...
Vercel Functions that accept OTEL spans from `@whoops/sdk` and fan them out to the dashboard SSE stream.
...
- Runs detectors from `@whoops/detectors` synchronously on each batch and persists hits.
